...
public class PaymentImpl implements Transaction {

    private final Logger logger = LoggerFactory.getLogger(PaymentImpl.class);

    @Override
    public String send(TransactionHelper transactionHelper) {

        logger.info("In PaymentImpl send");
        ...
        try {
            ...
            logger.debug("In PaymentImpl send. Message: "+ e.getMessage());
        }

        String requestQueueName = "switching-queue";

        if (transactionHelper.getType() == TransactionHelperType.EXTERNAL) {
            requestQueueName = "server-queue";
